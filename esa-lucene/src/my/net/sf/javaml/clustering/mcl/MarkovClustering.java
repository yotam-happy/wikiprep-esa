...
public class MarkovClustering {
    ...
    public SparseMatrix run(SparseMatrix a, double maxResidual, double pGamma, double loopGain, double maxZero) {
    	System.out.println("doing Markov Clustering");
        // add cycles
    	System.out.println("Adding loop gain & normalizing");
        addLoops(a, loopGain);
        a.normaliseRows();
        double residual = 1.;
        int i = 0;
        ...
        while (residual > maxResidual) {
        	...
        	System.out.println("Doing expand step...");
            a = expand(a);
        	System.out.println("Doing inflate step...");
    ...
    public double inflate(SparseMatrix m, double p, double zeromax) {
        ...
        TIntDoubleIterator it = rowsums.iterator();
        while(it.hasNext()){
        	it.advance();
            SparseVector row = m.get(it.key());
            double max = row.max();
